...
import { DATA, type Certification } from "@/data/resume";
...
function CertificationHighlight({ certification }: { certification: Certification }) {
  ...
  const className = "flex items-start gap-2.5 rounded-md border border-primary/20 bg-primary/[0.035] px-3 py-2";

  return certification.href ? (
    <Link href={certification.href} target="_blank" rel="noopener noreferrer" aria-label={`Verify ${certification.name}`} className={`${className} transition-colors hover:border-primary/35 hover:bg-primary/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`}>
      {content}
    </Link>
        ...
        <div className="mx-auto flex w-full max-w-2xl flex-col justify-between gap-2 gap-y-6 md:flex-row">
          <div className="order-2 min-w-0 max-w-[560px] space-y-4 md:order-1">
            ...
            <div className="grid max-w-xl grid-cols-1 gap-2 pt-2 text-sm sm:grid-cols-2">
              {DATA.certifications.slice(0, 2).map((certification) => <CertificationHighlight key={certification.name} certification={certification} />)}
            </div>
          </div>
          <BlurFade delay={BLUR_FADE_DELAY} className="order-1 mx-auto md:order-2 md:mx-0 md:pt-1">
            <Avatar className="size-24 rounded-full border border-primary/20 bg-primary/5 shadow-lg ring-4 ring-muted md:size-32 dark:bg-primary/10">
              <AvatarImage alt={`${DATA.name} profile photo`} src={DATA.avatarUrl} className="bg-white object-cover" />
